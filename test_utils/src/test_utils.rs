...
pub fn _prep_test_dir(
    ...
    // Separate out the name of the function, which is now the last element in
    // this path.
    ...
    // directory to copy to get all files/subdirectories.
...
// Testing with logs is subtle. If logs won't be examined by unit tests, this is
...
// [test-log](https://crates.io/crates/test-log) seems like a possibility, but
// it works only for `env_logger`. While `rstest` offers fixtures, this seems
// like a bit of overkill to call one function for each test.
...
// This avoids a lot of clippy warnings: "unnecessary use of `to_string`", for a
// line like this:
//
